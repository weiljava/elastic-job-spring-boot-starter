...
public class JobConfParser implements ApplicationContextAware {
    ...
    public void setApplicationContext(ApplicationContext ctx) throws BeansException {
        ...
        for (Object confBean : beanMap.values()) {
            ...
            // 解决CGLIB代理和继承的问题
            Class<?>[] interfacesClass = getFirstInterfacesClass(clz);
            String jobTypeName = interfacesClass[0].getSimpleName();
            ...
            String executorServiceHandler = getEnvironmentStringValue(jobName, JobAttributeTag.EXECUTOR_SERVICE_HANDLER,
                    conf.executorServiceHandler());

            String jobShardingStrategyClass =
                    getEnvironmentStringValue(jobName, JobAttributeTag.JOB_SHARDING_STRATEGY_CLASS,
                            conf.jobShardingStrategyClass());
            String eventTraceRdbDataSource =
                    getEnvironmentStringValue(jobName, JobAttributeTag.EVENT_TRACE_RDB_DATA_SOURCE,
            ...
            int reconcileIntervalMinutes = getEnvironmentIntValue(jobName, JobAttributeTag.RECONCILE_INTERVAL_MINUTES,
            ...
            if (StringUtils.hasText(eventTraceRdbDataSource)) {
                BeanDefinitionBuilder rdbFactory =
                        BeanDefinitionBuilder.rootBeanDefinition(JobEventRdbConfiguration.class);
            ...
            defaultListableBeanFactory
                    .registerBeanDefinition(jobName + "SpringJobScheduler", factory.getBeanDefinition());
    ...
    private Class<?>[] getFirstInterfacesClass(Class<?> clz) {
        Class<?>[] interfacesClass = clz.getInterfaces();
        if (interfacesClass == null || interfacesClass.length == 0) {
            Class<?> superClass = clz.getSuperclass();
            interfacesClass = getFirstInterfacesClass(superClass);
        }
    ...
    private List<BeanDefinition> getTargetElasticJobListeners(ElasticJobConf conf) {
        ...
        long startedTimeoutMilliseconds =
                getEnvironmentLongValue(conf.name(), JobAttributeTag.DISTRIBUTED_LISTENER_STARTED_TIMEOUT_MILLISECONDS,
